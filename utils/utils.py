...
def Z_anchor_score(anchor, gZ):

    return torch.abs(anchor-gZ)
...
def build_targets(pred_uvZQ, pred_cls, target, anchors, ignore_thres):
    ...
    ByteTensor = torch.cuda.ByteTensor if pred_uvZQ.is_cuda else torch.ByteTensor
    FloatTensor = torch.cuda.FloatTensor if pred_uvZQ.is_cuda else torch.FloatTensor
    ...
    tQw = FloatTensor(nB, nA, nG, nG).fill_(0)
    ...
    gZ = target[:, 4]
    gQ = target[:, 5:]
    # Get anchors with best score (L1 distance)
    z_score = torch.stack([Z_anchor_score(anchor, gZ) for anchor in anchors])
    best_score, best_n = z_score.min(0)
    # Separate target values
    b, target_labels = target[:, :2].long().t()
    gu, gv = guv.t()
    gZ = gZ
    gQw, gQx, gQy, gQz = gQ.t()
    gi, gj = guv.long().t()
    ...
    tQx[b, best_n, gj, gi] = gQx
    tQy[b, best_n, gj, gi] = gQy
